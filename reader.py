...
# Dataset to be used in RNN training and CGMH pipeline
class dataset(object):
    ...
    # When called, returns a batch with 'batch_size' data. If step > batch_num,
    # loop around to the beginning
    def __call__(self, batch_size, step):
        batch_num = self.length//batch_size
        step = step % batch_num
        # Returns the 
        ret_input = self.input[step * batch_size: (step + 1) * batch_size]
